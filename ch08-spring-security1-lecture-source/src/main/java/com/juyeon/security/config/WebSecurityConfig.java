package com.juyeon.security.config;
...
//            registry.anyRequest().permitAll();  // 모든 요청 허용
    ...
    * (임시로 메모리상에 사용자 등록해서 테스트하기)
    * - 사용자: UserDetails 타입
    * - 사용자를 조회하는 서비스: UserDetailsService
    * */
    @Bean
    public UserDetailsService /* => 인증 객체 */ userDetailsService() {
        // 일반사용자
        ...
        return new InMemoryUserDetailsManager(user,admin);  // 메모리상에 넘겨줌 (테스트할때만 씀)
    }
}
